...
from table2json_extractor.data_processing import (
    parse_documents,
    append_tables
)
...
@st.fragment
def displayTablePreview(df, num_rows):
    """
    ...
    if st.button("Refresh Table Preview"):
            pass  # Pressing the button will cause the function to rerun
...
def main():
    ...
    if uploaded_files:
        ...
        try:
            ...
            st.write(f"{total_tables} tables")
            ...
            if 'method' not in st.session_state['user_inputs']['table_selection']:
                st.session_state['user_inputs']['table_selection']['method'] = 'append tables'
    ...
    if profiles:
        ...
        if selected_profile_name != "-- Select --":
            ...
            if 'table_selection' not in user_inputs:
                st.warning("Loaded profile is missing 'table_selection'. Assigning default method 'append tables'.")
                user_inputs['table_selection'] = {'method': 'append tables'}
            else:
                # Ensure 'method' is set and valid
                method = user_inputs['table_selection'].get('method')
                if not method or method.lower() not in ['append tables', 'indexing', 'keyword', 'regex', 'criteria', 'saved_profile']:
                    st.warning(f"Loaded profile has invalid or missing 'method'. Assigning default method 'append tables'.")
                    user_inputs['table_selection']['method'] = 'append tables'
    ...
    st.divider()
    st.header("2. Extraction Parameters")

    # Table Selection Criteria
    st.subheader("Table Selection Criteria")

    # Define selection methods with proper mapping
    selection_methods_display = ['Append Tables', 'Indexing', 'Keyword', 'Regex', 'Criteria']
    selection_methods_values = ['append tables', 'indexing', 'keyword', 'regex', 'criteria']
    method_display_to_value = dict(zip(selection_methods_display, selection_methods_values))
    method_value_to_display = dict(zip(selection_methods_values, selection_methods_display))

    # Determine default method index
    selection_method_default = user_inputs.get('table_selection', {}).get('method', 'append tables')

    if selection_method_default in selection_methods_values:
        method_index = selection_methods_values.index(selection_method_default)
    else:
        method_index = 0  # default to 'append tables'

    # Use the display names in the selectbox
    selection_method_display = st.selectbox(
        "Select Table Selection Method",
        selection_methods_display,
        index=method_index
    )

    # Get the method value from the display name
    selection_method = method_display_to_value[selection_method_display]
    ...
    if selection_method.lower() == 'append tables':
        st.session_state['user_inputs']['table_selection'] = {
            'method': selection_method
            # No additional parameters needed for 'append tables'
        }
        # Append all tables into one table
        all_tables = st.session_state['extracted_tables']
        appended_table = append_tables(all_tables)
        selected_tables = [appended_table]
        selected_document_table_mapping = [(None, appended_table)]
        st.session_state['selected_tables_indices'] = [0]  # Only one table after appending
    else:
        st.session_state['user_inputs']['table_selection'] = {
    ...
    header_rows = st.number_input("Number of Header Rows", min_value=0, value=st.session_state['user_inputs'].get('header_rows', 0))

    # Update header_rows based on selection method
    if selection_method.lower() == 'append tables':
        header_rows = 0
        st.write("Note: When appending tables, header rows are set to 0 to prevent data misalignment.")
    ...
    if selection_method.lower() == 'append tables':
        # Append all tables into one table
        all_tables = st.session_state['extracted_tables']
        appended_table = append_tables(all_tables, header_rows=0)
        selected_tables = [appended_table]
        selected_document_table_mapping = [(None, appended_table)]
        st.session_state['selected_tables_indices'] = [0]  # Only one table after appending

        st.subheader("Appended Table Preview:")
        df = table_to_dataframe(appended_table)
        num_rows = st.number_input("Number of rows to display in preview:", min_value=1, max_value=len(df), value=5)
        displayTablePreview(df, num_rows)
    else:
        # Proceed with displaying selected tables and allow user to select which ones to include
        table_indices = []
        table_previews = []
        for idx, (doc_idx, table) in enumerate(st.session_state['document_table_mapping']):
    ...
    st.divider()
    st.header("3. Final Configurations")
    # Resource Limits
...
def save_profile(profile_name, profile_data):
    ...
    if 'table_selection' not in profile_data:
        profile_data['table_selection'] = {'method': 'append tables'}
    else:
        # Ensure 'method' is set and valid
